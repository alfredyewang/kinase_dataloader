...
class Dataset(tud.Dataset):
    ...
    def __init__(self, smiles_list,target_list,activity_list, vocabulary, tokenizer):
    ...
    def __getitem__(self, i):
        ...
        activity = self._activity_list[i]
        tokens = self._tokenizer.tokenize(smi)
        encoded = self._vocabulary.encode(tokens)
        return [torch.tensor(encoded, dtype=torch.long), target, activity] # pylint: disable=E1102
